...
def extract_weather_data(**context):
    ...
    response = http_hook.run(endpoint='/data/2.5/weather?q=Portland&APPID=6233159967eb45f342f3c82ab2c9d6ac')
    data = response.json()
...
def transform_load_data(task_instance):
    ...
    # ⚠️ Best practice: store credentials in Airflow Connections instead of code
    aws_credentials = {
    ...
    df_data.to_csv(f"s3://open-wheather-project-api/{file_name}", index=False, storage_options=aws_credentials)
...
with DAG(
    ...
    schedule_interval='@daily',  # <-- fixed key
    catchup=False
) as dag:

    is_weather_api_ready = HttpSensor(
        task_id='is_weather_api_
